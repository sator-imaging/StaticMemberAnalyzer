...
> [!NOTE]
> An `if` statement (with or without an `else` clause) that is the last non-empty statement at the method root level or loop root level is exempted from this exit completeness check.
